...
async function lackfat() {
    ...
    const lackfat = await noticonmonths.count({
        where: {
            nameMon0thandyear: {
                [Op.like]: `${numberofPreviousMonth}%`
        },
...
function getLast7Days() {
 ...
 async function checklistlackfat(id) {
    const token = await axios.post(`https://api.line.me/v2/bot/message/push`, {
        to: code,
        messages: [
            {
                "type": "text",
                "text": "เดือนนี้คุณทานไขมัน ไม่ถึงเป้าหมายหลายวันเลยนะ ಠ_ಠ"
            },
            {
                "type":"image",
