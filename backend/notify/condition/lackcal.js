...
async function lackofcalnoti() {
    ...
    const lackcal = await noticonmonths.count({
        where: {
            nameMon0thandyear: {
                [Op.like]: `${numberofPreviousMonth}%`
        },
...
function getLast7Days() {
 ...
 async function checklistlackcal(id) {
    const token = await axios.post(`https://api.line.me/v2/bot/message/push`, {
        to: code,
        messages: [
            {
                "type": "text",
                "text": "เดือนนี้คุณทาน calories ไม่ถึงเป้าหมายมาหลายวันเเล้ว ทานน้อยไปจะเกิดความเหนื่อยล้า เนื่องจากปริมาณแคลอรี่ที่ไม่เพียงพอ"
            },
            {
                "type":"image",
