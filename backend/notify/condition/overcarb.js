...
async function overcarb() {
    ...
    const overcarb = await noticonmonths.count({
        where: {
            nameMon0thandyear: {
                [Op.like]: `${numberofPreviousMonth}%`
        },
...
function getLast7Days() {
 ...
 async function checklistovercarb(id) {
    const token = await axios.post(`https://api.line.me/v2/bot/message/push`, {
        to: code,
        messages: [
            {
                "type": "text",
                "text": "เดือนนี้คุณทานเเป้งเกินค่อนข้างเยอะ ลองเช็คร่างกายดูสิ"
            },
            {
                "type":"image",
            ...
            {
                "type": "text",
                "text": "คาร์โบไฮเดรตกินมากไปอาจเป็น โรคอ้วน รวมถึงทำให้ระดับน้ำตาลกลูโคสในเลือดสูงขึ้น ซึ่งจะเสี่ยงเป็นเบาหวาน (つ﹏<。) "
            },
        ]
    }, {
        headers: {
            ...
            if(!checknotiscon){
                        const token = await axios.post(`https://api.line.me/v2/bot/message/push`, {
                            ...
                            messages: [
                                {
                                    "type": "text",
                                    "text": `${name} สัปดาห์นี้คุณกินแป้งเยอะหลายวันเเล้วน้า~ `
                                },
                                {
                                    "type":"image",
